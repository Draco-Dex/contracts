...
contract SwapTest is IGridSwapCallback, AbstractPayments {
    ...
    constructor(address _factory, address _weth9) AbstractPayments(_factory, _weth9) {}
    ...
    function gridexSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata _data) external override {
        ...
        if (amount0Delta > 0) pay(gridKey.token0, decodeData.payer, _msgSender(), uint256(amount0Delta));

        if (amount1Delta > 0) pay(gridKey.token1, decodeData.payer, _msgSender(), uint256(amount1Delta));
    }
